...
def add_observables(sbml_model):
    ...
    for p in par_list:
        if p.startswith('observable_Mclp_'):
            sbml_model.removeParameter(p)
            sbml_model.removeRuleByVariable(p)
    ...
    r.setFormula(re.sub('scaling_proliferation_reference', 'observableParameter1_viability', r.getFormula()))
